                ...
                <div class="lg:col-span-1">
                    <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/50 p-4">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-2">
                            @php
                                $total = count($peserta);
                                $hadir = collect($peserta)->filter(function($p) use ($materi) {
                                    $attendance = $p->absensi->where('materi_id', $materi->id)->first();
                                    return $attendance && $attendance->status === 'hadir';
                                })->count();
                                $terlambat = collect($peserta)->filter(function($p) use ($materi) {
                                    $attendance = $p->absensi->where('materi_id', $materi->id)->first();
                                    return $attendance && $attendance->status === 'terlambat';
                                })->count();
                                $belum_absen = collect($peserta)->filter(function($p) use ($materi) {
                                    $attendance = $p->absensi->where('materi_id', $materi->id)->first();
                            ...
                            <table class="min-w-full divide-y divide-gray-200 text-sm">
                                <thead class="bg-gray-50 sticky top-0">
                                    <tr>
                                        ...
                                        <th class="py-2 px-3 text-left text-xs font-semibold text-gray-900 uppercase">Delegasi</th>
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition-colors duration-200">
                                        ...
                                        <td class="py-2 px-3 text-xs">
                                            <span class="px-2 py-1 inline-flex items-center text-xs leading-4 font-semibold rounded-full
